...
namespace Sitecore.Support.Cintel
{
    ...
    {
        private static byte[] _bookmark;

        public static void SetBookmark(byte[] value)
        {
            _bookmark = value;
        }
        ...
        private async Task<ResultSet<List<IContactSearchResult>>> FindAsync(ContactSearchParameters parameters)
        {
            ...
            using (var client = XConnect.Client.Configuration.SitecoreXConnectClientConfiguration.GetClient())
            {
                ...
                SetBookmark(batchEnumerator.Result.GetBookmark());
        ...
        private Task<IAsyncEntityBatchEnumerator<Contact>> QueryIndex(IAsyncQueryable<Contact> contacts, ContactSearchParameters parameters, string[] facets)
        {
            ...
            IAsyncQueryable<Contact> query = null;
            Expression<Func<Contact, bool>> predicateExpression = null;
            ...
            Fetch(GetResult(contacts, facets, interactionExpandOptions, 20).Result);
            ...
            Fetch(GetResult(query, facets, interactionExpandOptions, 20).Result);
            ...
            var channelFilters = parameters.AdditionalParameters.ContainsKey(SearchFiltersParams.Channel_Param)
                ? (parameters.AdditionalParameters[SearchFiltersParams.Channel_Param] as List<SearchItem>) : null;
            ...
            predicateExpression = null;
            var campaignFilters = parameters.AdditionalParameters.ContainsKey(SearchFiltersParams.Campaign_Param)
                ? (parameters.AdditionalParameters[SearchFiltersParams.Campaign_Param] as List<SearchItem>) : null;
            if (campaignFilters != null && campaignFilters.Count != 0)
            {
                foreach (SearchItem campaignItem in campaignFilters)
                {
                    Guid campaignDefinitionId = new Guid(campaignItem.ItemId);
                    if (predicateExpression == null)
                        predicateExpression = c => c.InteractionsCache()
                            .InteractionCaches.Any(ic => ic.DefinitionIds.Any(d => d == campaignDefinitionId));
                    else
                        predicateExpression = predicateExpression.Or(c => c.InteractionsCache()
                            .InteractionCaches.Any(ic => ic.DefinitionIds.Any(d => d == campaignDefinitionId)));
                }
            ...
            var outcomeFilters = parameters.AdditionalParameters.ContainsKey(SearchFiltersParams.Outcome_Param)
                ? (parameters.AdditionalParameters[SearchFiltersParams.Outcome_Param] as List<SearchItem>) : null;
            ...
            var goalFilters = parameters.AdditionalParameters.ContainsKey(SearchFiltersParams.Goal_Param)
                ? (parameters.AdditionalParameters[SearchFiltersParams.Goal_Param] as List<SearchItem>) : null;
            ...
            Fetch(GetResult(query, facets, interactionExpandOptions, 20).Result);
            return GetResult(query, facets, new RelatedInteractionsExpandOptions(IpInfo.DefaultFacetKey
                , ProfileScores.DefaultFacetKey
                , UserAgentInfo.DefaultFacetKey)
            {
                StartDateTime = parameters.FromDate,
        ...
        private Task<IAsyncEntityBatchEnumerator<Contact>> GetResult(IAsyncQueryable<Contact> query, string[] facets, RelatedInteractionsExpandOptions interactionExpandOptions, int pageSize)
        {
            var batchEnumerator = query
                ...
                }).GetBatchEnumerator(GetBookmark(), pageSize);

            return batchEnumerator;
        }

        private static IReadOnlyCollection<Contact> Fetch(IAsyncEntityBatchEnumerator<Contact> query)
        {
            query.MoveNext();
